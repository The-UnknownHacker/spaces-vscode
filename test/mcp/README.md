# Spaces Development MCP Server

This directory contains a Model Context Protocol (MCP) server that provides Playwright browser automation capabilities for Spaces development and testing. The MCP server exposes Spaces's Playwright testing infrastructure through a standardized interface, allowing AI assistants and other tools to interact with browsers programmatically.
...
│   ├── playwright.ts        # Spaces Playwright integration
...
- **Spaces Integration**: Uses Spaces's existing Playwright test infrastructure
...
### Server Won't Start
- Ensure Spaces has been built and run at least once (via F5 or `code.sh`)
...
This MCP server is part of the Spaces development infrastructure. When making changes:
